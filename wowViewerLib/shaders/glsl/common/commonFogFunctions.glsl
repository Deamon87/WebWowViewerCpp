...
vec4 makeFog2(const in PSFog mainFogData, const in PSFog fogDataSecond, in vec4 final, in vec3 uViewUp, in vec3 vertexInViewSpaceMy, in vec3 sunDirInViewSpace, in int blendMode) {
    ...
    vec4 classicFogParams = fogDataSecond.classicFogParams;
    ...
    {
        // Block 743
        vec4 densityParams = fogDataSecond.densityParams;
        float start = densityParams.x;
        float end = densityParams.y;
        float density = densityParams.z;
        float bias = densityParams.w;
        float heightRate = fogDataSecond.color_and_heightRate.w;
        float heightDensity = fogDataSecond.heightDensity_and_endColor.x;
        float fogZScalar = fogDataSecond.sunDirection_and_fogZScalar.w;
        vec4 t755 = fogDataSecond.mainFogEndDist_mainFogStartDist_legacyFogScalar_blendAlpha;
        ...
        vec4 t785 = fogDataSecond.heightPlane;
        ...
        vec4 t793 = fogDataSecond.heightFogCoeff;
        ...
        vec4 t826 = fogDataSecond.mainFogCoeff;
        ...
        vec4 t842 = fogDataSecond.heightDensityFogCoeff;
    ...
    float t924 = fogDataSecond.sunAngle_and_sunColor.x;
    float alpha_935 = final.w;
    float end2_938 = saturatef(((vLength - fogDataSecond.heightFogEndColor_fogStartOffset.w) / fogDataSecond.heightColor_and_endFogDistance.w));
    float end_940 = (end2_938 * (end2_938 * end2_938));
    vec3 heightColor_942 = mix(mainFogData.heightColor_and_endFogDistance.xyz, mainFogData.heightFogEndColor_fogStartOffset.xyz, vec3(end2_938));
    float saturate_943 = saturatef(end_940);
    vec3 fogFinal_945 = mix(mainFogData.color_and_heightRate.xyz, mainFogData.heightDensity_and_endColor.yzw, vec3(saturate_943));
    vec3 fogFinal_947 = mix(fogFinal_945, heightColor_942, vec3(heightFog_955));
    float dot_948 = dot(normalize_918, fogDataSecond.sunDirection_and_fogZScalar.xyz);
    float nDotSun_949 = saturatef(dot_948);
    vec3 sunColor_951 = mix(fogFinal_947, mainFogData.sunAngle_and_sunColor.yzw, vec3(fogDataSecond.sunPercentage.x));
    ...
    vec4 outColor_986 = (outColor_713 + (t982 * fogDataSecond.mainFogEndDist_mainFogStartDist_legacyFogScalar_blendAlpha.w));
