...
string WebGLSLCompiler::sanitize_underscores(const string &str)
{
    string res;
...
void WebGLSLCompiler::remap_ext_framebuffer_fetch(uint32_t input_attachment_index, uint32_t color_location)
{
...
string WebGLSLCompiler::buffer_to_packing_standard(const SPIRType &type, bool support_std430_without_scalar_layout)
{
...
void WebGLSLCompiler::emit_buffer_reference_block(SPIRType &type, bool forward_declaration)
{
    string buffer_name;
...
void WebGLSLCompiler::declare_undefined_values()
{
    bool emitted = false;
...
string WebGLSLCompiler::to_rerolled_array_expression(const string &base_expr, const SPIRType &type)
{
...
string WebGLSLCompiler::to_composite_constructor_expression(uint32_t id, bool uses_buffer_offset)
{
    auto &type = expression_type(id);
                                      ...
                                      bool suppress_usage_tracking)
{
    ...
    else
    {
        ...
        //Hook data for condition replacements
        auto varName = to_name(result_id);
        tempVarDeclMap[varName] = rhs;
...
bool WebGLSLCompiler::check_explicit_lod_allowed(uint32_t lod)
{
    auto &execution = get_entry_point();
...
string WebGLSLCompiler::legacy_tex_op(const std::string &op, const SPIRType &imgtype, uint32_t lod, uint32_t tex)
{
    const char *type;
...
void WebGLSLCompiler::prepare_access_chain_for_scalar_access(std::string &, const SPIRType &, spv::StorageClass, bool &)
{
}
...
const char *WebGLSLCompiler::flags_to_qualifiers_glsl(const SPIRType &type, const Bitset &flags)
{
...
const char *WebGLSLCompiler::to_precision_qualifiers_glsl(uint32_t id)
{
    auto &type = expression_type(id);
...
string WebGLSLCompiler::type_to_array_glsl(const SPIRType &type)
{
...
string WebGLSLCompiler::image_type_glsl(const SPIRType &type, uint32_t id)
{
...
void WebGLSLCompiler::branch(BlockID from, uint32_t cond, BlockID true_block, BlockID false_block)
{
    ...
    if (true_sub)
    {
        ...
        auto condStr = to_expression(cond);
        ...
        if (!defineSubstituteRequired) {
            //check tempVarDeclMap
            condStr = tempVarDeclMap[condStr];
            defineSubstituteRequired =
                (condStr.find("uPixelShader") != std::string::npos) ||
                (condStr.find("uVertexShader") != std::string::npos) ;
        }
...
void WebGLSLCompiler::emit_array_copy(const string &lhs, uint32_t rhs_id, StorageClass, StorageClass)
{
...
void WebGLSLCompiler::bitcast_from_builtin_load(uint32_t source_id, std::string &expr, const SPIRType &expr_type)
{
...
void WebGLSLCompiler::bitcast_to_builtin_store(uint32_t target_id, std::string &expr, const SPIRType &expr_type)
{
    // Only interested in standalone builtin variables.
...
void WebGLSLCompiler::convert_non_uniform_expression(const SPIRType &type, std::string &expr)
{
...
void WebGLSLCompiler::emit_block_hints(const SPIRBlock &)
{
}
...
void WebGLSLCompiler::propagate_nonuniform_qualifier(uint32_t id)
{
