...
const char *WebGLSLCompiler::vector_swizzle(int vecsize, int index)
{
...
void WebGLSLCompiler::reset(uint32_t iteration_count)
{
...
void WebGLSLCompiler::remap_pls_variables()
{
...
void WebGLSLCompiler::remap_ext_framebuffer_fetch(uint32_t input_attachment_index, uint32_t color_location, bool coherent)
{
...
bool WebGLSLCompiler::location_is_framebuffer_fetch(uint32_t location) const
{
...
bool WebGLSLCompiler::location_is_non_coherent_framebuffer_fetch(uint32_t location) const
{
	return std::find_if(begin(inout_color_attachments), end(inout_color_attachments),
	                    [&](const std::pair<uint32_t, bool> &elem) {
...
void WebGLSLCompiler::find_static_extensions()
{
...
void WebGLSLCompiler::require_polyfill(Polyfill polyfill, bool relaxed)
{
...
void WebGLSLCompiler::ray_tracing_khr_fixup_locations()
{
	uint32_t location = 0;
...
string WebGLSLCompiler::compile()
{
...
std::string WebGLSLCompiler::get_partial_source()
{
	return buffer.str();
}

void WebGLSLCompiler::build_workgroup_size(SmallVector<string> &arguments, const SpecializationConstant &wg_x,
...
void WebGLSLCompiler::request_subgroup_feature(ShaderSubgroupSupportHelper::Feature feature)
{
	if (options.vulkan_semantics)
	{
...
void WebGLSLCompiler::emit_header()
{
	auto &execution = get_entry_point();
...
bool WebGLSLCompiler::type_is_empty(const SPIRType &type)
{
	return type.basetype == SPIRType::Struct && type.member_types.empty();
}

void WebGLSLCompiler::emit_struct(SPIRType &type)
{
...
string WebGLSLCompiler::to_interpolation_qualifiers(const Bitset &flags)
{
	string res;
...
string WebGLSLCompiler::layout_for_member(const SPIRType &type, uint32_t index)
{
	if (is_legacy())
		return "";
...
const char *WebGLSLCompiler::format_to_glsl(spv::ImageFormat format)
{
...
uint32_t WebGLSLCompiler::type_to_packed_base_size(const SPIRType &type, BufferPackingStandard)
{
...
uint32_t WebGLSLCompiler::type_to_packed_alignment(const SPIRType &type, const Bitset &flags,
...
uint32_t WebGLSLCompiler::type_to_packed_array_stride(const SPIRType &type, const Bitset &flags,
                                                   BufferPackingStandard packing)
{
...
uint32_t WebGLSLCompiler::type_to_packed_size(const SPIRType &type, const Bitset &flags, BufferPackingStandard packing)
{
	// If using PhysicalStorageBufferEXT storage class, this is a pointer,
	// and is 64-bit.
	if (is_physical_pointer(type))
...
bool WebGLSLCompiler::buffer_is_packing_standard(const SPIRType &type, BufferPackingStandard packing,
...
bool WebGLSLCompiler::can_use_io_location(StorageClass storage, bool block)
{
...
string WebGLSLCompiler::layout_for_variable(const SPIRVariable &var)
{
...
string WebGLSLCompiler::buffer_to_packing_standard(const SPIRType &type,
...
void WebGLSLCompiler::emit_push_constant_block(const SPIRVariable &var)
{
...
void WebGLSLCompiler::emit_push_constant_block_vulkan(const SPIRVariable &var)
{
	emit_buffer_block(var);
}

void WebGLSLCompiler::emit_push_constant_block_glsl(const SPIRVariable &var)
{
...
void WebGLSLCompiler::emit_buffer_block(const SPIRVariable &var)
{
	auto &type = get<SPIRType>(var.basetype);
...
void WebGLSLCompiler::emit_buffer_block_legacy(const SPIRVariable &var)
{
	auto &type = get<SPIRType>(var.basetype);
...
void WebGLSLCompiler::emit_buffer_reference_block(uint32_t type_id, bool forward_declaration)
{
...
void WebGLSLCompiler::emit_buffer_block_native(const SPIRVariable &var)
{
	auto &type = get<SPIRType>(var.basetype);
...
void WebGLSLCompiler::emit_buffer_block_flattened(const SPIRVariable &var)
{
	auto &type = get<SPIRType>(var.basetype);
...
const char *WebGLSLCompiler::to_storage_qualifiers_glsl(const SPIRVariable &var)
{
	auto &execution = get_entry_point();
...
void WebGLSLCompiler::emit_flattened_io_block_member(const std::string &basename, const SPIRType &type, const char *qual,
...
void WebGLSLCompiler::emit_flattened_io_block_struct(const std::string &basename, const SPIRType &type, const char *qual,
                                                  const SmallVector<uint32_t> &indices)
{
...
void WebGLSLCompiler::emit_flattened_io_block(const SPIRVariable &var, const char *qual)
{
...
void WebGLSLCompiler::emit_interface_block(const SPIRVariable &var)
{
	auto &type = get<SPIRType>(var.basetype);
...
void WebGLSLCompiler::emit_uniform(const SPIRVariable &var)
{
	auto &type = get<SPIRType>(var.basetype);
...
string WebGLSLCompiler::constant_value_macro_name(uint32_t id)
{
	return join("SPIRV_CROSS_CONSTANT_ID_", id);
}

void WebGLSLCompiler::emit_specialization_constant_op(const SPIRConstantOp &constant)
{
...
int WebGLSLCompiler::get_constant_mapping_to_workgroup_component(const SPIRConstant &c) const
{
...
void WebGLSLCompiler::emit_constant(const SPIRConstant &constant)
{
...
void WebGLSLCompiler::emit_entry_point_declarations()
{
}

void WebGLSLCompiler::replace_illegal_names(const unordered_set<string> &keywords)
{
...
void WebGLSLCompiler::replace_illegal_names()
{
	// clang-format off
...
void WebGLSLCompiler::replace_fragment_output(SPIRVariable &var)
{
...
void WebGLSLCompiler::replace_fragment_outputs()
{
	ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable &var) {
...
string WebGLSLCompiler::remap_swizzle(const SPIRType &out_type, uint32_t input_components, const string &expr)
{
...
void WebGLSLCompiler::fixup_image_load_store_access()
{
...
bool WebGLSLCompiler::should_force_emit_builtin_block(StorageClass storage)
{
...
void WebGLSLCompiler::fixup_implicit_builtin_block_names(ExecutionModel model)
{
	ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable &var) {
		auto &type = this->get<SPIRType>(var.basetype);
		bool block = has_decoration(type.self, DecorationBlock);
...
void WebGLSLCompiler::emit_declared_builtin_block(StorageClass storage, ExecutionModel model)
{
...
bool WebGLSLCompiler::variable_is_lut(const SPIRVariable &var) const
{
...
void WebGLSLCompiler::emit_resources()
{
	auto &execution = get_entry_point();
...
void WebGLSLCompiler::emit_output_variable_initializer(const SPIRVariable &var)
{
...
void WebGLSLCompiler::emit_subgroup_arithmetic_workaround(const std::string &func, Op op, GroupOperation group_op)
{
	std::string result;
	switch (group_op)
	{
...
void WebGLSLCompiler::emit_extension_workarounds(spv::ExecutionModel model)
{
...
void WebGLSLCompiler::emit_polyfills(uint32_t polyfills, bool relaxed)
{
	const char *qual = "";
...
string WebGLSLCompiler::to_func_call_arg(const SPIRFunction::Parameter &, uint32_t id)
{
...
void WebGLSLCompiler::force_temporary_and_recompile(uint32_t id)
{
...
uint32_t WebGLSLCompiler::consume_temporary_in_precision_context(uint32_t type_id, uint32_t id, Options::Precision precision)
{
...
void WebGLSLCompiler::handle_invalid_expression(uint32_t id)
{
...
string WebGLSLCompiler::unpack_expression_type(string expr_str, const SPIRType &, uint32_t, bool, bool)
{
	return expr_str;
}

// Sometimes we proactively enclosed an expression where it turns out we might have not needed it after all.
void WebGLSLCompiler::strip_enclosed_expression(string &expr)
{
...
bool WebGLSLCompiler::needs_enclose_expression(const std::string &expr)
{
...
string WebGLSLCompiler::enclose_expression(const string &expr)
{
...
string WebGLSLCompiler::dereference_expression(const SPIRType &expr_type, const std::string &expr)
{
...
string WebGLSLCompiler::address_of_expression(const std::string &expr)
{
...
string WebGLSLCompiler::to_enclosed_expression(uint32_t id, bool register_expression_read)
{
...
string WebGLSLCompiler::to_unpacked_row_major_matrix_expression(uint32_t id)
{
...
string WebGLSLCompiler::to_unpacked_expression(uint32_t id, bool register_expression_read)
{
...
string WebGLSLCompiler::to_enclosed_unpacked_expression(uint32_t id, bool register_expression_read)
{
	return enclose_expression(to_unpacked_expression(id, register_expression_read));
}

string WebGLSLCompiler::to_dereferenced_expression(uint32_t id, bool register_expression_read)
{
...
string WebGLSLCompiler::to_pointer_expression(uint32_t id, bool register_expression_read)
{
	auto &type = expression_type(id);
...
string WebGLSLCompiler::to_enclosed_pointer_expression(uint32_t id, bool register_expression_read)
{
	auto &type = expression_type(id);
	if (is_pointer(type) && expression_is_lvalue(id) && !should_dereference(id))
		return address_of_expression(to_enclosed_expression(id, register_expression_read));
...
string WebGLSLCompiler::to_extract_component_expression(uint32_t id, uint32_t index)
{
...
string WebGLSLCompiler::to_extract_constant_composite_expression(uint32_t result_type, const SPIRConstant &c,
...
string WebGLSLCompiler::to_rerolled_array_expression(const SPIRType &parent_type,
...
string WebGLSLCompiler::to_composite_constructor_expression(const SPIRType &parent_type, uint32_t id, bool block_like_type)
{
	auto &type = expression_type(id);
...
string WebGLSLCompiler::to_non_uniform_aware_expression(uint32_t id)
{
...
string WebGLSLCompiler::to_expression(uint32_t id, bool register_expression_read)
{
...
SmallVector<ConstantID> WebGLSLCompiler::get_composite_constant_ids(ConstantID const_id)
{
...
void WebGLSLCompiler::fill_composite_constant(SPIRConstant &constant, TypeID type_id,
...
void WebGLSLCompiler::set_composite_constant(ConstantID const_id, TypeID type_id,
                                          const SmallVector<ConstantID> &initializers)
{
...
TypeID WebGLSLCompiler::get_composite_member_type(TypeID type_id, uint32_t member_idx)
{
	auto &type = get<SPIRType>(type_id);
	if (is_array(type))
...
string WebGLSLCompiler::constant_op_expression(const SPIRConstantOp &cop)
{
...
string WebGLSLCompiler::constant_expression(const SPIRConstant &c,
...
string WebGLSLCompiler::convert_half_to_string(const SPIRConstant &c, uint32_t col, uint32_t row)
{
	string res;
...
string WebGLSLCompiler::convert_float_to_string(const SPIRConstant &c, uint32_t col, uint32_t row)
{
	string res;
...
std::string WebGLSLCompiler::convert_double_to_string(const SPIRConstant &c, uint32_t col, uint32_t row)
{
	string res;
...
string WebGLSLCompiler::constant_expression_vector(const SPIRConstant &c, uint32_t vector)
{
	auto type = get<SPIRType>(c.constant_type);
	type.columns = 1;
...
SPIRExpression &WebGLSLCompiler::emit_uninitialized_temporary_expression(uint32_t type, uint32_t id)
{
	forced_temporaries.insert(id);
...
void WebGLSLCompiler::emit_uninitialized_temporary(uint32_t result_type, uint32_t result_id)
{
...
string WebGLSLCompiler::declare_temporary(uint32_t result_type, uint32_t result_id)
{
	auto &type = get<SPIRType>(result_type);
...
bool WebGLSLCompiler::expression_is_forwarded(uint32_t id) const
{
	return forwarded_temporaries.count(id) != 0;
}

bool WebGLSLCompiler::expression_suppresses_usage_tracking(uint32_t id) const
{
	return suppressed_usage_tracking.count(id) != 0;
}

bool WebGLSLCompiler::expression_read_implies_multiple_reads(uint32_t id) const
{
...
SPIRExpression &WebGLSLCompiler::emit_op(uint32_t result_type, uint32_t result_id, const string &rhs, bool forwarding,
...
void WebGLSLCompiler::emit_unary_op(uint32_t result_type, uint32_t result_id, uint32_t op0, const char *op)
{
...
void WebGLSLCompiler::emit_unary_op_cast(uint32_t result_type, uint32_t result_id, uint32_t op0, const char *op)
{
	auto &type = get<SPIRType>(result_type);
	bool forward = should_forward(op0);
...
void WebGLSLCompiler::emit_mesh_tasks(SPIRBlock &block)
{
...
void WebGLSLCompiler::emit_binary_op(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1, const char *op)
{
...
void WebGLSLCompiler::emit_unrolled_unary_op(uint32_t result_type, uint32_t result_id, uint32_t operand, const char *op)
{
	auto &type = get<SPIRType>(result_type);
...
void WebGLSLCompiler::emit_unrolled_binary_op(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1,
...
SPIRType WebGLSLCompiler::binary_op_bitcast_helper(string &cast_op0, string &cast_op1, SPIRType::BaseType &input_type,
...
bool WebGLSLCompiler::emit_complex_bitcast(uint32_t result_type, uint32_t id, uint32_t op0)
{
...
void WebGLSLCompiler::emit_binary_op_cast(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1,
...
void WebGLSLCompiler::emit_unary_func_op(uint32_t result_type, uint32_t result_id, uint32_t op0, const char *op)
{
	bool forward = should_forward(op0);
...
void WebGLSLCompiler::emit_binary_func_op(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1,
                                       const char *op)
{
...
void WebGLSLCompiler::emit_atomic_func_op(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1,
                                       const char *op)
{
	auto &type = get<SPIRType>(result_type);
	if (type_is_floating_point(type))
...
void WebGLSLCompiler::emit_atomic_func_op(uint32_t result_type, uint32_t result_id,
...
void WebGLSLCompiler::emit_unary_func_op_cast(uint32_t result_type, uint32_t result_id, uint32_t op0, const char *op,
...
void WebGLSLCompiler::emit_trinary_func_op_bitextract(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1,
...
void WebGLSLCompiler::emit_trinary_func_op_cast(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1,
...
void WebGLSLCompiler::emit_binary_func_op_cast_clustered(uint32_t result_type, uint32_t result_id, uint32_t op0,
...
void WebGLSLCompiler::emit_binary_func_op_cast(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1,
...
void WebGLSLCompiler::emit_trinary_func_op(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1,
                                        uint32_t op2, const char *op)
{
...
void WebGLSLCompiler::emit_quaternary_func_op(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1,
...
void WebGLSLCompiler::emit_bitfield_insert_op(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1,
                                           uint32_t op2, uint32_t op3, const char *op,
...
string WebGLSLCompiler::legacy_tex_op(const std::string &op, const SPIRType &imgtype, uint32_t tex)
{
	const char *type;
...
bool WebGLSLCompiler::to_trivial_mix_op(const SPIRType &type, string &op, uint32_t left, uint32_t right, uint32_t lerp)
{
...
string WebGLSLCompiler::to_ternary_expression(const SPIRType &restype, uint32_t select, uint32_t true_value,
...
void WebGLSLCompiler::emit_mix_op(uint32_t result_type, uint32_t id, uint32_t left, uint32_t right, uint32_t lerp)
{
	auto &lerptype = expression_type(lerp);
...
string WebGLSLCompiler::to_combined_image_sampler(VariableID image_id, VariableID samp_id)
{
...
bool WebGLSLCompiler::is_supported_subgroup_op_in_opengl(spv::Op op, const uint32_t *ops)
{
	switch (op)
	{
...
void WebGLSLCompiler::emit_sampled_image_op(uint32_t result_type, uint32_t result_id, uint32_t image_id, uint32_t samp_id)
{
...
void WebGLSLCompiler::emit_sparse_feedback_temporaries(uint32_t result_type_id, uint32_t id, uint32_t &feedback_id,
                                                    uint32_t &texel_id)
{
...
uint32_t WebGLSLCompiler::get_sparse_feedback_texel_id(uint32_t id) const
{
...
void WebGLSLCompiler::emit_texture_op(const Instruction &i, bool sparse)
{
	auto *ops = stream(i);
...
std::string WebGLSLCompiler::to_texture_op(const Instruction &i, bool sparse, bool *forward,
                                        SmallVector<uint32_t> &inherited_expressions)
{
	auto *ops = stream(i);
	auto op = static_cast<Op>(i.op);
...
bool WebGLSLCompiler::expression_is_constant_null(uint32_t id) const
{
...
bool WebGLSLCompiler::expression_is_non_value_type_array(uint32_t ptr)
{
...
string WebGLSLCompiler::to_function_name(const TextureFunctionNameArguments &args)
{
	if (args.has_min_lod)
	{
		if (options.es)
...
std::string WebGLSLCompiler::convert_separate_image_to_expression(uint32_t id)
{
...
string WebGLSLCompiler::to_function_args(const TextureFunctionArguments &args, bool *p_forward)
{
...
Op WebGLSLCompiler::get_remapped_spirv_op(Op op) const
{
...
GLSLstd450 WebGLSLCompiler::get_remapped_glsl_op(GLSLstd450 std450_op) const
{
...
void WebGLSLCompiler::emit_glsl_op(uint32_t result_type, uint32_t id, uint32_t eop, const uint32_t *args, uint32_t length)
{
...
void WebGLSLCompiler::emit_nminmax_op(uint32_t result_type, uint32_t id, uint32_t op0, uint32_t op1, GLSLstd450 op)
{
...
void WebGLSLCompiler::emit_emulated_ahyper_op(uint32_t result_type, uint32_t id, uint32_t op0, GLSLstd450 op)
{
...
void WebGLSLCompiler::emit_spv_amd_shader_ballot_op(uint32_t result_type, uint32_t id, uint32_t eop, const uint32_t *args,
                                                 uint32_t)
{
...
void WebGLSLCompiler::emit_spv_amd_shader_explicit_vertex_parameter_op(uint32_t result_type, uint32_t id, uint32_t eop,
...
void WebGLSLCompiler::emit_spv_amd_shader_trinary_minmax_op(uint32_t result_type, uint32_t id, uint32_t eop,
                                                         const uint32_t *args, uint32_t)
{
...
void WebGLSLCompiler::emit_spv_amd_gcn_shader_op(uint32_t result_type, uint32_t id, uint32_t eop, const uint32_t *args,
                                              uint32_t)
{
...
void WebGLSLCompiler::emit_subgroup_op(const Instruction &i)
{
...
string WebGLSLCompiler::bitcast_glsl_op(const SPIRType &out_type, const SPIRType &in_type)
{
...
string WebGLSLCompiler::bitcast_glsl(const SPIRType &result_type, uint32_t argument)
{
...
std::string WebGLSLCompiler::bitcast_expression(SPIRType::BaseType target_type, uint32_t arg)
{
...
std::string WebGLSLCompiler::bitcast_expression(const SPIRType &target_type, SPIRType::BaseType expr_type,
...
string WebGLSLCompiler::builtin_to_glsl(BuiltIn builtin, StorageClass storage)
{
	switch (builtin)
	{
...
const char *WebGLSLCompiler::index_to_swizzle(uint32_t index)
{
	switch (index)
	{
	case 0:
...
void WebGLSLCompiler::access_chain_internal_append_index(std::string &expr, uint32_t /*base*/, const SPIRType * /*type*/,
...
bool WebGLSLCompiler::access_chain_needs_stage_io_builtin_translation(uint32_t)
{
	return true;
}

string WebGLSLCompiler::access_chain_internal(uint32_t base, const uint32_t *indices, uint32_t count,
...
void WebGLSLCompiler::check_physical_type_cast(std::string &, const SPIRType *, uint32_t)
{
}

bool WebGLSLCompiler::prepare_access_chain_for_scalar_access(std::string &, const SPIRType &, spv::StorageClass, bool &)
{
	return false;
}

string WebGLSLCompiler::to_flattened_struct_member(const string &basename, const SPIRType &type, uint32_t index)
{
...
uint32_t WebGLSLCompiler::get_physical_type_stride(const SPIRType &) const
{
	SPIRV_CROSS_THROW("Invalid to call get_physical_type_stride on a backend without native pointer support.");
}

string WebGLSLCompiler::access_chain(uint32_t base, const uint32_t *indices, uint32_t count, const SPIRType &target_type,
...
string WebGLSLCompiler::load_flattened_struct(const string &basename, const SPIRType &type)
{
	auto expr = type_to_glsl_constructor(type);
	expr += '(';
...
std::string WebGLSLCompiler::to_flattened_access_chain_expression(uint32_t id)
{
...
void WebGLSLCompiler::store_flattened_struct(const string &basename, uint32_t rhs_id, const SPIRType &type,
                                          const SmallVector<uint32_t> &indices)
{
...
void WebGLSLCompiler::store_flattened_struct(uint32_t lhs_id, uint32_t value)
{
...
std::string WebGLSLCompiler::flattened_access_chain(uint32_t base, const uint32_t *indices, uint32_t count,
...
std::string WebGLSLCompiler::flattened_access_chain_struct(uint32_t base, const uint32_t *indices, uint32_t count,
...
std::string WebGLSLCompiler::flattened_access_chain_matrix(uint32_t base, const uint32_t *indices, uint32_t count,
                                                        const SPIRType &target_type, uint32_t offset,
...
std::string WebGLSLCompiler::flattened_access_chain_vector(uint32_t base, const uint32_t *indices, uint32_t count,
                                                        const SPIRType &target_type, uint32_t offset,
                                                        uint32_t matrix_stride, bool need_transpose)
{
...
std::pair<std::string, uint32_t> WebGLSLCompiler::flattened_access_chain_offset(
...
bool WebGLSLCompiler::should_dereference(uint32_t id)
{
...
bool WebGLSLCompiler::should_forward(uint32_t id) const
{
...
bool WebGLSLCompiler::should_suppress_usage_tracking(uint32_t id) const
{
...
void WebGLSLCompiler::track_expression_read(uint32_t id)
{
	switch (ir.ids[id].get_type())
	{
	case TypeExpression:
...
bool WebGLSLCompiler::args_will_forward(uint32_t id, const uint32_t *args, uint32_t num_args, bool pure)
{
...
void WebGLSLCompiler::register_impure_function_call()
{
...
void WebGLSLCompiler::register_call_out_argument(uint32_t id)
{
	register_write(id);
...
string WebGLSLCompiler::variable_decl_function_local(SPIRVariable &var)
{
...
void WebGLSLCompiler::emit_variable_temporary_copies(const SPIRVariable &var)
{
	// Ensure that we declare phi-variable copies even if the original declaration isn't deferred
...
void WebGLSLCompiler::flush_variable_declaration(uint32_t id)
{
	// Ensure that we declare phi-variable copies even if the original declaration isn't deferred
	auto *var = maybe_get<SPIRVariable>(id);
...
bool WebGLSLCompiler::remove_duplicate_swizzle(string &op)
{
...
bool WebGLSLCompiler::remove_unity_swizzle(uint32_t base, string &op)
{
	auto pos = op.find_last_of('.');
	if (pos == string::npos || pos == 0)
		return false;
...
string WebGLSLCompiler::build_composite_combiner(uint32_t return_type, const uint32_t *elems, uint32_t length)
{
	ID base = 0;
	string op;
	string subop;
...
bool WebGLSLCompiler::skip_argument(uint32_t id) const
{
...
bool WebGLSLCompiler::optimize_read_modify_write(const SPIRType &type, const string &lhs, const string &rhs)
{
...
void WebGLSLCompiler::register_control_dependent_expression(uint32_t expr)
{
...
void WebGLSLCompiler::emit_block_instructions(SPIRBlock &block)
{
...
void WebGLSLCompiler::disallow_forwarding_in_expression_chain(const SPIRExpression &expr)
{
...
void WebGLSLCompiler::handle_store_to_invariant_variable(uint32_t store_id, uint32_t value_id)
{
...
void WebGLSLCompiler::emit_store_statement(uint32_t lhs_expression, uint32_t rhs_expression)
{
...
uint32_t WebGLSLCompiler::get_integer_width_for_instruction(const Instruction &instr) const
{
	if (instr.length < 3)
		return 32;
...
uint32_t WebGLSLCompiler::get_integer_width_for_glsl_instruction(GLSLstd450 op, const uint32_t *ops, uint32_t length) const
{
	if (length < 1)
		return 32;
...
void WebGLSLCompiler::forward_relaxed_precision(uint32_t dst_id, const uint32_t *args, uint32_t length)
{
...
WebGLSLCompiler::Options::Precision WebGLSLCompiler::analyze_expression_precision(const uint32_t *args, uint32_t length) const
{
...
void WebGLSLCompiler::analyze_precision_requirements(uint32_t type_id, uint32_t dst_id, uint32_t *args, uint32_t length)
{
	if (!backend.requires_relaxed_precision_analysis)
		return;
...
WebGLSLCompiler::TemporaryCopy WebGLSLCompiler::handle_instruction_precision(const Instruction &instruction)
{
...
void WebGLSLCompiler::emit_instruction(const Instruction &instruction)
{
...
void WebGLSLCompiler::append_global_func_args(const SPIRFunction &func, uint32_t index, SmallVector<string> &arglist)
{
...
string WebGLSLCompiler::to_member_name(const SPIRType &type, uint32_t index)
{
	if (type.type_alias != TypeID(0) &&
	    !has_extended_decoration(type.type_alias, SPIRVCrossDecorationBufferBlockRepacked))
	{
...
void WebGLSLCompiler::add_member_name(SPIRType &type, uint32_t index)
{
	auto &memb = ir.meta[type.self].members;
	if (index < memb.size() && !memb[index].alias.empty())
	{
...
bool WebGLSLCompiler::is_non_native_row_major_matrix(uint32_t id)
{
...
bool WebGLSLCompiler::member_is_non_native_row_major_matrix(const SPIRType &type, uint32_t index)
{
	// Natively supported row-major matrices do not need to be converted.
	if (backend.native_row_major_matrix && !is_legacy())
		return false;
...
bool WebGLSLCompiler::member_is_remapped_physical_type(const SPIRType &type, uint32_t index) const
{
...
bool WebGLSLCompiler::member_is_packed_physical_type(const SPIRType &type, uint32_t index) const
{
...
string WebGLSLCompiler::convert_row_major_matrix(string exp_str, const SPIRType &exp_type, uint32_t /* physical_type_id */,
...
bool WebGLSLCompiler::variable_decl_is_remapped_storage(const SPIRVariable &var, StorageClass storage) const
{
...
void WebGLSLCompiler::emit_struct_member(const SPIRType &type, uint32_t member_type_id, uint32_t index,
...
void WebGLSLCompiler::emit_struct_padding_target(const SPIRType &)
{
}

string WebGLSLCompiler::flags_to_qualifiers_glsl(const SPIRType &type, const Bitset &flags)
{
...
string WebGLSLCompiler::to_precision_qualifiers_glsl(uint32_t id)
{
	auto &type = expression_type(id);
...
void WebGLSLCompiler::fixup_io_block_patch_primitive_qualifiers(const SPIRVariable &var)
{
...
string WebGLSLCompiler::to_qualifiers_glsl(uint32_t id)
{
...
string WebGLSLCompiler::argument_decl(const SPIRFunction::Parameter &arg)
{
...
string WebGLSLCompiler::to_initializer_expression(const SPIRVariable &var)
{
	return to_unpacked_expression(var.initializer);
}

string WebGLSLCompiler::to_zero_initialized_expression(uint32_t type_id)
{
#ifndef NDEBUG
	auto &type = get<SPIRType>(type_id);
...
bool WebGLSLCompiler::type_can_zero_initialize(const SPIRType &type) const
{
	if (type.pointer)
		return false;
...
string WebGLSLCompiler::variable_decl(const SPIRVariable &variable)
{
...
const char *WebGLSLCompiler::to_pls_qualifiers_glsl(const SPIRVariable &variable)
{
...
string WebGLSLCompiler::pls_decl(const PlsRemap &var)
{
...
uint32_t WebGLSLCompiler::to_array_size_literal(const SPIRType &type) const
{
	return to_array_size_literal(type, uint32_t(type.array.size() - 1));
}

uint32_t WebGLSLCompiler::to_array_size_literal(const SPIRType &type, uint32_t index) const
{
...
string WebGLSLCompiler::to_array_size(const SPIRType &type, uint32_t index)
{
	assert(type.array.size() == type.array_size_literal.size());
...
string WebGLSLCompiler::type_to_array_glsl(const SPIRType &type, uint32_t)
{
...
string WebGLSLCompiler::image_type_glsl(const SPIRType &type, uint32_t id, bool /*member*/)
{
...
string WebGLSLCompiler::type_to_glsl_constructor(const SPIRType &type)
{
...
void WebGLSLCompiler::add_variable(unordered_set<string> &variables_primary,
...
void WebGLSLCompiler::add_local_variable_name(uint32_t id)
{
	add_variable(local_variable_names, block_names, ir.meta[id].decoration.alias);
}

void WebGLSLCompiler::add_resource_name(uint32_t id)
{
	add_variable(resource_names, block_names, ir.meta[id].decoration.alias);
}

void WebGLSLCompiler::add_header_line(const std::string &line)
{
	header_lines.push_back(line);
}

bool WebGLSLCompiler::has_extension(const std::string &ext) const
{
	auto itr = find(begin(forced_extensions), end(forced_extensions), ext);
	return itr != end(forced_extensions);
}

void WebGLSLCompiler::require_extension(const std::string &ext)
{
	if (!has_extension(ext))
		forced_extensions.push_back(ext);
}

const SmallVector<std::string> &WebGLSLCompiler::get_required_extensions() const
{
	return forced_extensions;
}

void WebGLSLCompiler::require_extension_internal(const string &ext)
{
...
void WebGLSLCompiler::flatten_buffer_block(VariableID id)
{
	auto &var = get<SPIRVariable>(id);
	auto &type = get<SPIRType>(var.basetype);
...
bool WebGLSLCompiler::builtin_translates_to_nonarray(spv::BuiltIn /*builtin*/) const
{
	return false; // GLSL itself does not need to translate array builtin types to non-array builtin types
}

bool WebGLSLCompiler::is_user_type_structured(uint32_t /*id*/) const
{
...
bool WebGLSLCompiler::check_atomic_image(uint32_t id)
{
	auto &type = expression_type(id);
...
void WebGLSLCompiler::emit_fixup()
{
...
void WebGLSLCompiler::flush_phi(BlockID from, BlockID to)
{
...
void WebGLSLCompiler::branch_to_continue(BlockID from, BlockID to)
{
...
void WebGLSLCompiler::branch(BlockID from, BlockID to)
{
	flush_phi(from, to);
...
void WebGLSLCompiler::branch(BlockID from, uint32_t cond, BlockID true_block, BlockID false_block)
{
	auto &from_block = get<SPIRBlock>(from);
...
string WebGLSLCompiler::emit_continue_block(uint32_t continue_block, bool follow_true_block, bool follow_false_block)
{
...
void WebGLSLCompiler::emit_while_loop_initializers(const SPIRBlock &block)
{
...
string WebGLSLCompiler::emit_for_loop_initializers(const SPIRBlock &block)
{
...
bool WebGLSLCompiler::for_loop_initializers_are_same_type(const SPIRBlock &block)
{
...
void WebGLSLCompiler::emit_block_instructions_with_masked_debug(SPIRBlock &block)
{
...
bool WebGLSLCompiler::attempt_emit_loop_header(SPIRBlock &block, SPIRBlock::Method method)
{
...
void WebGLSLCompiler::flush_undeclared_variables(SPIRBlock &block)
{
	for (auto &v : block.dominated_variables)
		flush_variable_declaration(v);
}

void WebGLSLCompiler::emit_hoisted_temporaries(SmallVector<pair<TypeID, ID>> &temporaries)
{
...
void WebGLSLCompiler::emit_block_chain(SPIRBlock &block)
{
...
void WebGLSLCompiler::begin_scope()
{
	statement("{");
	indent++;
}

void WebGLSLCompiler::end_scope()
{
	if (!indent)
...
void WebGLSLCompiler::end_scope(const string &trailer)
{
	if (!indent)
		SPIRV_CROSS_THROW("Popping empty indent stack.");
	indent--;
...
void WebGLSLCompiler::end_scope_decl()
{
	if (!indent)
		SPIRV_CROSS_THROW("Popping empty indent stack.");
	indent--;
...
void WebGLSLCompiler::end_scope_decl(const string &decl)
{
	if (!indent)
		SPIRV_CROSS_THROW("Popping empty indent stack.");
	indent--;
...
void WebGLSLCompiler::check_function_call_constraints(const uint32_t *args, uint32_t length)
{
...
const Instruction *WebGLSLCompiler::get_next_instruction_in_block(const Instruction &instr)
{
...
uint32_t WebGLSLCompiler::mask_relevant_memory_semantics(uint32_t semantics)
{
...
bool WebGLSLCompiler::emit_array_copy(const char *expr, uint32_t lhs_id, uint32_t rhs_id, StorageClass, StorageClass)
{
	string lhs;
	if (expr)
		lhs = expr;
...
bool WebGLSLCompiler::unroll_array_to_complex_store(uint32_t target_id, uint32_t source_id)
{
...
void WebGLSLCompiler::unroll_array_from_complex_load(uint32_t target_id, uint32_t source_id, std::string &expr)
{
	if (!backend.force_gl_in_out_block)
		return;
	// This path is only relevant for GL backends.
...
void WebGLSLCompiler::cast_from_variable_load(uint32_t source_id, std::string &expr, const SPIRType &expr_type)
{
...
SPIRType::BaseType WebGLSLCompiler::get_builtin_basetype(BuiltIn builtin, SPIRType::BaseType default_type)
{
	// TODO: Fill in for more builtins.
	switch (builtin)
	{
...
void WebGLSLCompiler::cast_to_variable_store(uint32_t target_id, std::string &expr, const SPIRType &expr_type)
{
...
void WebGLSLCompiler::convert_non_uniform_expression(string &expr, uint32_t ptr_id)
{
...
void WebGLSLCompiler::emit_block_hints(const SPIRBlock &block)
{
...
void WebGLSLCompiler::preserve_alias_on_reset(uint32_t id)
{
	preserved_aliases[id] = get_name(id);
}

void WebGLSLCompiler::reset_name_caches()
{
...
void WebGLSLCompiler::fixup_anonymous_struct_names(std::unordered_set<uint32_t> &visited, const SPIRType &type)
{
...
void WebGLSLCompiler::fixup_anonymous_struct_names()
{
...
void WebGLSLCompiler::fixup_type_alias()
{
...
void WebGLSLCompiler::reorder_type_alias()
{
...
void WebGLSLCompiler::emit_line_directive(uint32_t file_id, uint32_t line_literal)
{
...
void WebGLSLCompiler::emit_copy_logical_type(uint32_t lhs_id, uint32_t lhs_type_id, uint32_t rhs_id, uint32_t rhs_type_id,
...
bool WebGLSLCompiler::subpass_input_is_framebuffer_fetch(uint32_t id) const
{
...
const SPIRVariable *WebGLSLCompiler::find_subpass_input_by_attachment_index(uint32_t index) const
{
...
const SPIRVariable *WebGLSLCompiler::find_color_output_by_location(uint32_t location) const
{
	const SPIRVariable *ret = nullptr;
	ir.for_each_typed_id<SPIRVariable>([&](uint32_t, const SPIRVariable &var) {
...
void WebGLSLCompiler::emit_inout_fragment_outputs_copy_to_subpass_inputs()
{
	for (auto &remap : subpass_to_framebuffer_fetch_attachment)
	{
...
const char *WebGLSLCompiler::ShaderSubgroupSupportHelper::get_extension_name(Candidate c)
{
...
SmallVector<std::string> WebGLSLCompiler::ShaderSubgroupSupportHelper::get_extra_required_extension_names(Candidate c)
{
	switch (c)
	{
...
const char *WebGLSLCompiler::ShaderSubgroupSupportHelper::get_extra_required_extension_predicate(Candidate c)
{
	switch (c)
	{
	case ARB_shader_ballot:
...
WebGLSLCompiler::ShaderSubgroupSupportHelper::FeatureVector WebGLSLCompiler::ShaderSubgroupSupportHelper::
...
WebGLSLCompiler::ShaderSubgroupSupportHelper::FeatureMask WebGLSLCompiler::ShaderSubgroupSupportHelper::
    get_feature_dependency_mask(Feature feature)
{
	return build_mask(get_feature_dependencies(feature));
}

bool WebGLSLCompiler::ShaderSubgroupSupportHelper::can_feature_be_implemented_without_extensions(Feature feature)
{
...
WebGLSLCompiler::ShaderSubgroupSupportHelper::Candidate WebGLSLCompiler::ShaderSubgroupSupportHelper::
...
void WebGLSLCompiler::ShaderSubgroupSupportHelper::request_feature(Feature feature)
{
	feature_mask |= (FeatureMask(1) << feature) | get_feature_dependency_mask(feature);
}

bool WebGLSLCompiler::ShaderSubgroupSupportHelper::is_feature_requested(Feature feature) const
{
	return (feature_mask & (1u << feature)) != 0;
}

WebGLSLCompiler::ShaderSubgroupSupportHelper::Result WebGLSLCompiler::ShaderSubgroupSupportHelper::resolve() const
{
	Result res;
...
WebGLSLCompiler::ShaderSubgroupSupportHelper::CandidateVector WebGLSLCompiler::ShaderSubgroupSupportHelper::
...
WebGLSLCompiler::ShaderSubgroupSupportHelper::FeatureMask WebGLSLCompiler::ShaderSubgroupSupportHelper::build_mask(
...
WebGLSLCompiler::ShaderSubgroupSupportHelper::Result::Result()
{
...
void WebGLSLCompiler::request_workaround_wrapper_overload(TypeID id)
{
...
void WebGLSLCompiler::rewrite_load_for_wrapped_row_major(std::string &expr, TypeID loaded_type, ID ptr)
{
...
void WebGLSLCompiler::mask_stage_output_by_location(uint32_t location, uint32_t component)
{
	masked_output_locations.insert({ location, component });
}

void WebGLSLCompiler::mask_stage_output_by_builtin(BuiltIn builtin)
{
	masked_output_builtins.insert(builtin);
}

bool WebGLSLCompiler::is_stage_output_variable_masked(const SPIRVariable &var) const
{
	auto &type = get<SPIRType>(var.basetype);
	bool is_block = has_decoration(type.self, DecorationBlock);
...
bool WebGLSLCompiler::is_stage_output_block_member_masked(const SPIRVariable &var, uint32_t index, bool strip_array) const
{
	auto &type = get<SPIRType>(var.basetype);
	bool is_block = has_decoration(type.self, DecorationBlock);
	if (!is_block)
...
bool WebGLSLCompiler::is_per_primitive_variable(const SPIRVariable &var) const
{
...
bool WebGLSLCompiler::is_stage_output_location_masked(uint32_t location, uint32_t component) const
{
	return masked_output_locations.count({ location, component }) != 0;
}

bool WebGLSLCompiler::is_stage_output_builtin_masked(spv::BuiltIn builtin) const
{
	return masked_output_builtins.count(builtin) != 0;
}

uint32_t WebGLSLCompiler::get_declared_member_location(const SPIRVariable &var, uint32_t mbr_idx, bool strip_array) const
{
...
uint32_t WebGLSLCompiler::get_accumulated_member_location(const SPIRVariable &var, uint32_t mbr_idx, bool strip_array) const
{
...
StorageClass WebGLSLCompiler::get_expression_effective_storage_class(uint32_t ptr)
{
	auto *var = maybe_get_backing_variable(ptr);
...
uint32_t WebGLSLCompiler::type_to_location_count(const SPIRType &type) const
{
	uint32_t count;
	if (type.basetype == SPIRType::Struct)
	{
...
std::string WebGLSLCompiler::format_float(float value) const
{
	if (float_formatter)
...
std::string WebGLSLCompiler::format_double(double value) const
{
	if (float_formatter)
