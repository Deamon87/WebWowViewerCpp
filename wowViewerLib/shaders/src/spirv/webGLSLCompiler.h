//
// Created by deamon on 27.11.19.
//

#ifndef AWEBWOWVIEWERCPP_WEBGLSLCOMPILER_H
#define AWEBWOWVIEWERCPP_WEBGLSLCOMPILER_H
...
namespace SPIRV_CROSS_NAMESPACE {
    ...
    class WebGLSLCompiler : public Compiler {
    ...
    public:
        ...
        explicit WebGLSLCompiler(std::vector<uint32_t>
        spirv_)
        :
        ...
        WebGLSLCompiler(const uint32_t *ir_, size_t word_count)
            : Compiler(ir_, word_count) {
            init();
        }

        explicit WebGLSLCompiler(const ParsedIR &ir_)
            : Compiler(ir_) {
            init();
        }

        explicit WebGLSLCompiler(ParsedIR
        &&ir_)
        :
    ...
    protected:
        ...
        virtual void
        emit_spv_amd_shader_explicit_vertex_parameter_op(uint32_t result_type, uint32_t result_id, uint32_t op,
                                                         const uint32_t *args, uint32_t count);
        ...
        virtual void
        emit_spv_amd_gcn_shader_op(uint32_t result_type, uint32_t result_id, uint32_t op, const uint32_t *args,
                                   uint32_t count);
        ...
        virtual void
        emit_sampled_image_op(uint32_t result_type, uint32_t result_id, uint32_t image_id, uint32_t samp_id);
        ...
        virtual std::string
        unpack_expression_type(std::string expr_str, const SPIRType &type, uint32_t physical_type_id,
        ...
        void
        emit_binary_func_op_cast(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1, const char *op,
                                 SPIRType::BaseType input_type, bool skip_cast_if_equal_type);

        void
        emit_trinary_func_op_cast(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1, uint32_t op2,
        ...
        void
        emit_unrolled_binary_op(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1, const char *op,
        ...
        std::string
        access_chain_internal(uint32_t base, const uint32_t *indices, uint32_t count, AccessChainFlags flags,
        ...
        flattened_access_chain_offset(const SPIRType &basetype, const uint32_t *indices,
        ...
        std::string
        bitcast_expression(const SPIRType &target_type, SPIRType::BaseType expr_type, const std::string &expr);
        ...
        uint32_t
        get_integer_width_for_glsl_instruction(GLSLstd450 op, const uint32_t *arguments, uint32_t length) const;
...
#endif //AWEBWOWVIEWERCPP_WEBGLSLCOMPILER_H
