...
static GLuint sizeFromUniformType(GLint type)
{
    GLuint s;
...
GShaderPermutationGL4x::GShaderPermutationGL4x(std::string &shaderName, IDevice * device) : m_device(device), m_shaderName(shaderName){
...
void GShaderPermutationGL4x::setUnf(const std::string &name, GLuint index)  {
    const char * cstr = name.c_str();
    m_uniformMap[CalculateFNV(cstr)] = index;
}

void GShaderPermutationGL4x::bindProgram() {
    glUseProgram(m_programBuffer);
}
void GShaderPermutationGL4x::unbindProgram() {
    glUseProgram(0);
}