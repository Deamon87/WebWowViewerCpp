...
#include "FrameBasedHeapAllocator.h"
#include <atomic>
#include <vector>
#include <array>
...
class FrameHeapRegion {
public:
    FrameHeapRegion(size_t size) : m_buffer(size) {
...
std::array<std::list<std::unique_ptr<FrameHeapRegion>>, IDevice::MAX_FRAMES_IN_FLIGHT> g_frameHeapList;
...
void * frameAllocate(size_t size) {
    ...
    auto &current_list = g_frameHeapList[currentFrame];
    ...
    if (ptr == nullptr) {
        ...
        current_list.push_back(std::make_unique<FrameHeapRegion>(newSize));
...
//    auto &current_list = frames[currentFrame];
    ...
    for (auto &frameList : g_frameHeapList) {
