...
namespace sqlite_orm {
    ...
    namespace internal {
        ...
        template<class T>
        SQLITE_ORM_INLINE_VAR constexpr bool
            is_operator_argument_v<T, std::enable_if_t<polyfill::is_specialization_of_v<T, expression_t>>> = true;
...
namespace sqlite_orm {
    ...
    namespace internal {
        ...
        struct virtual_table_t : basic_table {
            ...
            template<class L, satisfies<std::is_member_pointer, L> = true>
            const std::string* find_column_name(L m) const {
                const std::string* res = nullptr;
                for_each_column([&res, m](auto& c) {
                                  if(compare_any(c.member_pointer, m) || compare_any(c.setter, m)) {
                                      res = &c.name;
                                  }
                              });
